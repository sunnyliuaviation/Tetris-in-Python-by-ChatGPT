...
# Define the path to the music and sound files
music_game = os.path.join(current_directory, 'Music', 'game.wav')
music_game_over = os.path.join(current_directory, 'Music', 'game_over.wav')
sound_bump = os.path.join(current_directory, 'Music', 'bump.wav')
sound_dispel = os.path.join(current_directory, 'Music', 'dispel.wav')
...
# Load sound effects
game_over_sound = pygame.mixer.Sound(music_game_over)
bump_sound = pygame.mixer.Sound(sound_bump)
dispel_sound = pygame.mixer.Sound(sound_dispel)
...
def lock_piece(piece):
    ...
    bump_sound.play()  # Play bump sound when piece locks
...
def clear_rows():
    ...
    for y in range(play_height):
        if all(grid[y]):
            ...
            grid[0] = [0] * play_width
    if full_rows > 0:
        dispel_sound.play()  # Play dispel sound when rows are cleared
    score += full_rows * 10
...
def draw_preview(piece):
    ...
    # Draw the preview piece in gray, but only in empty spaces
    for y in range(len(temp_piece.rotated_shape)):
        for x in range(len(temp_piece.rotated_shape[y])):
            if temp_piece.rotated_shape[y][x] != 0:
                ...
                if grid[temp_piece.y + y][temp_piece.x + x] == 0:
...
def game_loop():
    ...
    while not game_exit:
        if game_over:
            ...
            pygame.mixer.music.play(-1)  # Restart background music loop
            break
